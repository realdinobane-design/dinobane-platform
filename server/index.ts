...
app.use((req: any, res: any, next: any) => {
  ...
  if (!sid || !sessions.has(sid)) {
    sid = genSid();
    sessions.set(sid, {});
    res.setHeader("Set-Cookie", `dinobane_sid=${sid}; Path=/; HttpOnly; Max-Age=${7*24*3600}; SameSite=Lax`);
  }
  const store = sessions.get(sid)!;
  req.session = {
    get userId() { return store.userId; },
    set userId(v: number | undefined) { store.userId = v; },
    destroy(cb: () => void) { sessions.delete(sid); if (cb) cb(); },
  };
  next();
});
