...
// ─── STICKY X BANNER ──────────────────────────────────────────────────────────
function StickyXBanner() {
  return (
    <div className="sticky top-16 z-40 w-full bg-black border-b border-[#1a1a1a] shadow-md">
      <a
        href="https://x.com/realdinobane"
        target="_blank"
        rel="noopener noreferrer"
        className="flex items-center justify-between gap-4 px-4 sm:px-6 py-2.5 max-w-7xl mx-auto group hover:opacity-90 transition-opacity"
      >
        <div className="flex items-center gap-3">
          <div className="shrink-0 w-7 h-7 bg-white rounded-lg flex items-center justify-center">
            <svg viewBox="0 0 24 24" className="w-4 h-4 fill-black" xmlns="http://www.w3.org/2000/svg">
              <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.746l7.73-8.835L1.254 2.25H8.08l4.253 5.622 5.911-5.622Zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
            </svg>
          </div>
          <p className="text-white font-black text-sm leading-tight">Follow <span className="text-[#f5c842]">@realdinobane</span> on X for real-time updates</p>
        </div>
        <div className="shrink-0 flex items-center gap-1.5 bg-white text-black font-black text-xs px-3 py-1.5 rounded-full group-hover:bg-zinc-200 transition-colors">
          Follow
          <svg viewBox="0 0 24 24" className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
        </div>
      </a>
    </div>
  );
